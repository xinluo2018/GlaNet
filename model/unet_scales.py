...
class unet_local(nn.Module):
    def __init__(self, num_bands):
        ...
        self.down_conv1 = conv3x3_bn_relu(self.num_bands+1, 16)
        self.down_conv2 = conv3x3_bn_relu(16, 32)
        self.up_conv1 = conv3x3_bn_relu(32, 64)
        self.up_conv2 = conv3x3_bn_relu(80, 48)
        self.outlayer = nn.Sequential(
                                nn.Conv2d(49, 1, kernel_size=3, padding=1),
                                # nn.Sigmoid()
                                ) 

    def forward(self, x_img, x_pred_global):   ## input size: 6x256x256
        x = torch.cat([x_img, x_pred_global], dim=1)
        ## encoder part
        ...
        ## decoder part
        x2_up = self.up_conv1(x2)  ## 64x64x64
        x2_up = torch.cat([self.up(x2_up), x1], dim=1)      ## (64+16)x128x128
        x1_up = self.up_conv2(x2_up)      ## 48x128x128
        x1_up = torch.cat([self.up(x1_up), x_pred_global], dim=1)       ## (48+1)x256x256
        pro_pred = self.outlayer(x1_up)
        return pro_pred          
...
class unet_scales(nn.Module):
    def __init__(self, num_bands_local, 
        ...
        self.model_local = unet_local(num_bands_local)
    ...
    def forward(self, x_local, x_global):    
        ...
        prob_local = self.model_local(x_local, prob_global2local)
        return prob_local, prob_global, prob_global2local
