...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, attention_mask=None, attention_weights=None):
        ...
        # output
        out_vis = torch.matmul(att_vis, v_vis).permute(0, 2, 1, 3)  #  (b_s, nq, h, d_v)
        out_vis = out_vis.contiguous().view(b_s, nq, self.h * self.d_v)  # (b_s, nq, h*d_v)
        out_vis = self.resid_drop(self.out_proj_vis(out_vis))      # (b_s, nq, d_model)
        out_ir = torch.matmul(att_ir, v_ir).permute(0, 2, 1, 3)    # (b_s, nq, h, d_v)
        out_ir = out_ir.contiguous().view(b_s, nq, self.h * self.d_v)  # (b_s, nq, h*d_v)
...
class CrossTransformerBlock(nn.Module):
    def __init__(self, d_model, d_k, d_v, h, block_exp, attn_pdrop, resid_pdrop, loops_num=1):
        ...
        :param attn_pdrop: Dropout rate for attention
        :param resid_pdrop: Dropout rate for residuals
        :param loops_num: Number of times to repeat the cross attention and MLP blocks
        """
...
class TransformerFusionBlock(nn.Module):
    def __init__(self, d_model, vert_anchors=16, horz_anchors=16, h=8, block_exp=4, 
                 n_layer=1, embd_pdrop=0.1, attn_pdrop=0.1, resid_pdrop=0.1):
        ''' 
        :param d_model: Output dimensionality of the model
        :param vert_anchors: number of vertical anchors
        :param horz_anchors: number of horizontal anchors
        :param h: number of heads
        :param block_exp: expansion factor for MLP (feed foreword network)
        :param n_layer: number of transformer layers
        :param embd_pdrop: dropout rate for embedding
        :param attn_pdrop: dropout rate for attention
        :param resid_pdrop: dropout rate for residuals
        '''
    ...
    def forward(self, x):
        ...
        new_rgb_fea = self.vis_coefficient(self.avgpool(rgb_fea), self.maxpool(rgb_fea))  # SFS module
        new_c, new_h, new_w = new_rgb_fea.shape[1], new_rgb_fea.shape[2], new_rgb_fea.shape[3]
        rgb_fea_flat = new_rgb_fea.contiguous().view(bs, new_c, -1).permute(0, 2, 1) + self.pos_emb_vis # (b_s, nx, d)
        ...
        rgb_fea_flat, ir_fea_flat = self.crosstransformer([rgb_fea_flat, ir_fea_flat])   # ICFE module
        # rgb image
        ...
        new_rgb_fea = rgb_fea_CFE + rgb_fea  # skip connection 
        # ir image
        ...
        new_ir_fea = ir_fea_CFE + ir_fea  # skip connection
        ## feature fusion
...
if __name__ == "__main__":
    model = TransformerFusionBlock(d_model=64, vert_anchors=14, 
                                   horz_anchors=14, h=8, block_exp=4, 
                                   n_layer=1, embd_pdrop=0.1, 
                                   attn_pdrop=0.1, resid_pdrop=0.1)
    rgb_fea = torch.randn(2, 64, 512, 512) # dim:(b_s, c, h, w)
    ir_fea = torch.randn(2, 64, 512, 512) # dim:(b_s, c, h, w)
    out = model([rgb_fea, ir_fea])
    print(out.shape)  # Expected output shape: (2, 32, 512, 512)