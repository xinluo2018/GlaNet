...
class Cross_ViTransformerBlock_xyd(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, d:torch.Tensor):
        ...
        # d_att = shortcut_d + wd[1] * self.drop_path(d_att1) + wd[2] * self.drop_path(d_att2)
...
class Cross_Vit_Fusion(nn.Module):
    ...
    def forward(self, x, y, z):
        ...
        fus_xyz = torch.cat([x, y, z], dim=1)   # [B, 3*C, H, W]
        return fus_xyz
...
class Cross_SwinBasicLayer(nn.Module):
    ...
    def forward(self, x, y, d):
        ...
        x = x.transpose(1, 2).view(B, C, H, W)  # [B, C, H, W]
        y = y.transpose(1, 2).view(B, C, H, W)  # [B, C, H, W]
        d = d.transpose(1, 2).view(B, C, H, W)  # [B, C, H, W]
        fus_xyd = torch.cat([x, y, d], dim=1)   # [B, 3*C, H, W]
        return fus_xyd
...
class u3net_swin_fusion_(nn.Module):
    def __init__(self, backbone_name='resnet34',
        ...
        # self.aux3 = nn.Conv2d(self.out_channels[3]*3+self.decode_channels[0], 1, 3, padding=1)  # auxiliary output at layer 3
        # self.aux4 = nn.Conv2d(self.out_channels[4]*3, 1, 3, padding=1)  # auxiliary output at layer 4
        self.aux4 = nn.Conv2d(self.out_channels[4]*3, 1, 3, padding=1)  # auxiliary output at layer 3
    ...
    def forward(self, x, lat=None):       ## input size: 7x256x256
        ...
        # fea_opt, fea_dem = self.ssf_fusion_4(fea_opt, fea_dem)     # cross fusion of features at layer 4
        fea_fus_4 = self.cross_layers_4(fea_opt, fea_nir, fea_dem)   # cross attention fusion of features from three branches
        ...
        # skip_fea_opt = self.proj_y1_3(skip_fea_opt)  # project optical features to match decoder channels
        # skip_fea_dem = self.proj_y2_3(skip_fea_dem)  # project DEM features to match decoder channels
        # skip_fea_opt = self.cbam3_opt(skip_fea_opt)  # CBAM attention for optical features at layer 3
        # skip_fea_dem = self.cbam3_dem(skip_fea_dem)  # CBAM attention for DEM features at layer 3
        # fea_fus_3 = self.cross_layers_3(skip_fea_opt, skip_fea_nir, skip_fea_dem)   # cross attention fusion of skip features
        ...
        # fea_fus_2 = self.cross_layers_2(skip_fea_opt, skip_fea_nir, skip_fea_dem)  
        # fea_fus_2 = torch.cat([fea_fus_3, fea_fus_2], dim=1)  # concat skip features
        # skip_fea_opt, skip_fea_nir, skip_fea_dem = self.ssf_fusion_2(skip_fea_opt, skip_fea_nir, skip_fea_dem)  # cross fusion of features at layer 2
        fea_fus_2 = torch.cat([skip_fea_opt, skip_fea_nir, skip_fea_dem], dim=1)
        # fea_fus_2 = self.proj_feas_2(fea_fus_2)   
        # fea_fus_3, fea_fus_2 = self.ssf_fusion_2(fea_fus_3, fea_fus_2)
        fea_fus_2 = torch.cat([fea_fus_3, fea_fus_2], dim=1)  # concat skip features
        fea_fus_2 = self.DecoderBlocks[2](fea_fus_2)  # decode fused features
        # fea_fus_2 = self.decode2(fea_fus_2) 
        # out2 = self.aux2(fea_fus_2)   
        ...
        return logit, aux4_out # main output, auxiliary outputs for deep supervision
        # return logit

if __name__ == '__main__':
    model = u3net_swin_fusion_(
