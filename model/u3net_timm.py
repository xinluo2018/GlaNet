...
class u3net_timm(nn.Module):
    def __init__(self,  backbone_name='resnet34', 
        ...
        self.DecoderBlocks = nn.ModuleList([
                conv(self.out_channels[-1]*3, self.decode_channels[0]),   
                conv(self.out_channels[-2]*3+self.decode_channels[0], self.decode_channels[1]),  
                conv(self.out_channels[-3]*3+self.decode_channels[1], self.decode_channels[2]), 
                conv(self.out_channels[-4]*3+self.decode_channels[2], self.decode_channels[3]), 
                conv(self.out_channels[-5]*3+self.decode_channels[3], self.decode_channels[4])   
                ])
    ...
    def forward(self, x):       ## input size: 7x256x256
        ...
        fea_fus_3 = torch.cat([fea_fus_4, skip_fea_opt, skip_fea_nir, skip_fea_dem], dim=1)  # concat skip features
        ...
        fea_fus_2 = torch.cat([fea_fus_3, skip_fea_opt, skip_fea_nir, skip_fea_dem], dim=1)  # concat skip features
        ...
        fea_fus_1 = torch.cat([fea_fus_2, skip_fea_opt, skip_fea_nir, skip_fea_dem], dim=1)  # concat skip features
        ...
        fea_fus_0 = torch.cat([fea_fus_1, skip_fea_opt, skip_fea_nir, skip_fea_dem], dim=1)  # concat skip features
