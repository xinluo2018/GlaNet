...
class CMFNet(nn.Module):
    ...
    def __init__(self, num_bands_b1, num_bands_b2, patch_size=512):
        ...
        ##### RGB ENCODER ####
        self.conv1_1 = nn.Conv2d(num_bands_b1, 64, 3, padding=1)
        ...
        ##### DSM ENCODER ####
        self.conv1_1_d = nn.Conv2d(num_bands_b2, 64, kernel_size=3, padding=1)
        ...
        self.mtc = ChannelTransformer_cross(config_vit, vis, patch_size,
                                      channel_num=[64, 128, 256, 512],
                                      patchSize=config_vit.patch_sizes)
        self.mtc1 = ChannelTransformer(config_vit, vis, patch_size,
                                      channel_num=[64, 128, 256, 512],
                                      patchSize=config_vit.patch_sizes)
        ####  RGB DECODER  ####
        self.conv5_3_D = nn.Conv2d(512, 512, 3, padding=1)
        ...
        self.conv1_2_D = nn.Conv2d(64, 64, 3, padding=1)
        self.conv1_2_D_bn = nn.BatchNorm2d(64)
        self.conv1_1_D = nn.Conv2d(64, 1, 3, padding=1)
        self.last_layer = nn.Sigmoid()
        self.apply(self.weight_init)

    def forward(self, in_data):
        x, y = in_data[:, :self.num_bands_b1, :, :], in_data[:, self.num_bands_b1:, :, :]
        ########  DEPTH ENCODER  ########
        # Encoder block 1
        # y = y.unsqueeze(1)
        ...
        x = self.last_layer(self.conv1_1_D(x))
        return x

if __name__ == "__main__":
    x = torch.randn(2, 7, 256, 256)  
    model = CMFNet(num_bands_b1=6, num_bands_b2=1, patch_size=256)
    output = model(x)
    print(output.shape)
